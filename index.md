# IT Documentation

Welcome to the IT Documentation repository. This site contains IT policies and how-to guides for both Technical IT staff and end users.

## 📚 Documentation Sections

### [IT Policies](./policies/)

Policies and procedures for Technical IT staff covering security, operations, compliance, and infrastructure management.

[View Policies Index →](./policies/)

### [How-To Guides](./guides/)

Step-by-step guides organized by audience:
- **End User Guides** - For non-technical users
- **Technical Guides** - For IT staff

[View Guides Index →](./guides/)

## 🔍 Quick Navigation

- [All Policies](./policies/index.md)
- [End User Guides](./guides/end-users/)
- [Technical Guides](./guides/technical/)

## 📊 Documentation Status

Each section contains an index that tracks which documents are:
- ✅ **Written** - Complete and available
- 📝 **To Do** - Planned but not yet written

---

*Last updated: 2025*

